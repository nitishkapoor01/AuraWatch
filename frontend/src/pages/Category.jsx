  ...
  useEffect(() => {
    const fetchCategory = async () => {
      ...
      try {
        ...
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000/api'}`}/movies/search?query=${type === 'tv' ? 'show' : 'movie'}`);
        ...
        const trendingRes = await fetch(`${import.meta.env.VITE_API_BASE_URL || `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000/api'}`}/movies/trending`);
