...
const MovieDetail = () => {
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      ...
      try {
        ...
        const data = await fetchWithCache(`http://localhost:5000/api/movies/${id}?type=${type}`, onRevalidate);
        if (data && data.id) setMovie(data);
        else setMovie(null);
      } catch (error) {
    ...
    const fetchGlobalSettings = async () => {
      try {
        const res = await fetch('http://localhost:5000/api/settings/skip_ads_timer');
        if (res.ok) {
  ...
  useEffect(() => {
    ...
    const checkFavorite = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/favorites/check/${id}?type=${type}`, {
  ...
  useEffect(() => {
    ...
    const fetchSeasons = async () => {
      try {
        ...
        const data = await fetchWithCache(`http://localhost:5000/api/movies/${id}/seasons`, onRevalidate);
        if (Array.isArray(data)) {
          setSeasons(data);
  ...
  useEffect(() => {
    ...
    const fetchEpisodes = async () => {
      try {
        ...
        const data = await fetchWithCache(`http://localhost:5000/api/movies/${id}/season/${selectedSeason}`, onRevalidate);
        if (Array.isArray(data)) {
          setEpisodes(data);
  ...
  const handleWatchTrailer = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/movies/${id}/videos?type=${type}`);
      const data = await res.json();
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      if (isFavorite) {
        await fetch(`http://localhost:5000/api/favorites/${id}?type=${type}`, {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` }
        });
        setIsFavorite(false);
